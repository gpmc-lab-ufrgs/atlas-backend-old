...
class DistrictGeoJson(GeoJSONLayerView):
    ...
    def get_queryset(self):
        return self.model.objects.filter(Q(SIGLA_UF='RS') | Q(SIGLA_UF='RJ') | Q(SIGLA_UF='MG')) # return self.model.objects.filter(Q(SIGLA_UF='RS') | Q(SIGLA_UF='RJ')) filtra apenas os municípios do RS e RJ
